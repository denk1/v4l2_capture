...
namespace low_level
{
    VideoCapture::VideoCapture(const char *dev_name, const int height, const int width)
    {
        buff_frame.resize(height * width * 2);
        init_video_capture(dev_name);
    }
    ...
    void VideoCapture::init_video_capture(const char *device_name)
    {
    ...
    void VideoCapture::clear_video_capture()
    {
        stop_capturing();
        uninit_device();
        close_device();
    ...
    void VideoCapture::set_frame_buffer(const void *p, int size)
    {
        memcpy((void *)&buff_frame[0], p, size);
    ...
    int VideoCapture::read_frame(void)
    {
        ...
        case IO_METHOD_MMAP:
            ...
            set_frame_buffer(buffers[buf.index].start, buf.bytesused);
    ...
    void VideoCapture::init_userp(unsigned int buffer_size, const char *dev_name)
    {
        struct v4l2_requestbuffers req;
    ...
    void VideoCapture::usage(FILE *fp, int argc, char **argv)
    {
        fprintf(fp,
                "Usage: %s [options]\\n\\n"
                "Version 1.3\\n"
    ...
    const std::vector<unsigned char> &VideoCapture::get_buff()
    {
        return buff_frame;
    }
} // namespace low_level